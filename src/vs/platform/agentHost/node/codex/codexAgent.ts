...
import { codexAccountStateFromResponse, type ICodexAccountState } from './codexAccountState.js';
...
export class CodexAgent extends Disposable implements IAgent {
	...
	private async _startConnection(): Promise<IConnectionReady> {
		...
		const launchConfig = buildCodexLaunchConfig(process.env, proxyHandle, extraArgs);
